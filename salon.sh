...
MAIN_MENU() {
  ...
  if [[ -z $SERVICES ]]; then
    ...
    # If there are services, show them formatted
    ...
    if [[ ! $SERVICE_ID_SELECTED =~ ^[0-9]+$ ]]; then
      ...
      # If it's a number but not a valid service ID
      if [[ -z $VALID_SERVICE ]]; then
        ...
        # Check if the customer is new or not
        CUSTOMER_NAME=$($PSQL "SELECT name FROM customers WHERE phone = '$CUSTOMER_PHONE'")

        # If the customer is new
        if [[ -z $CUSTOMER_NAME ]]; then
          # Get the name and phone, then insert it into the table
